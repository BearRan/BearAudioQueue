...
#define kNumAQBufs 16           //  AudioQueue Buffer的个数
#define kAQDefaultBufSize 2048  //  AudioQueue Buffer的size
#define kAQMaxPacketDescs 512	// 用于存放描述文件的数组的个数
...
@interface AudioStreamer : NSObject
{
    ...
    NSThread *internalThread;			//  用于下载流文件的线程
    
    AudioQueueBufferRef audioQueueBuffer[kNumAQBufs];		// audio queue buffers
    AudioStreamPacketDescription packetDescs[kAQMaxPacketDescs];	// packet descriptions for enqueuing audio
    unsigned int fillBufferIndex;	// 被填充好数据的Buffer的索引
    UInt32 packetBufferSize;
    size_t bytesFilled;				// 已经被填满的字节数
    size_t packetsFilled;			// 已经被填满的包的数量
    bool inuse[kNumAQBufs];			// 表示Buffer仍在使用的标志位
    ...
    pthread_mutex_t queueBuffersMutex;			//  互斥锁，用于保护正在使用的线程
    pthread_cond_t queueBufferReadyCondition;	//  用于处理正在使用中的条件变量
    
    CFReadStreamRef stream;                     //  获取的文件流
    NSNotificationCenter *notificationCenter;
    
    UInt32 bitRate;				// Bits per second in the file
    NSInteger dataOffset;		// Offset of the first audio packet in the stream
    NSInteger fileLength;		// 文件的字节长度
    NSInteger seekByteOffset;	// 在文件中定位到某一字节，用于指定头文件的长度
